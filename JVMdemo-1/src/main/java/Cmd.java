...
public class Cmd {
    ...
    @Parameter(names = {"-?", "-help"}, description = "show help", order = 3, help = true)
    boolean helpFlag = false;
    //该属性通过@Parameter注解定义命令行参数为-version的时候，将versionFlag标记为true。
    @Parameter(names = "-version", description = "show version", order = 2)
    boolean versionFlag = false;
    //该属性通过@Parameter注解定义命令行参数为-cp/-classpath的时候，用classpath记录路径。
    @Parameter(names = {"-cp", "-classpath"}, description = "show classPath", order = 1)
    String classpath;
    //该属性存储命令行传入的所有参数。
